...
I think the GraphB+ algorithm leaves a bit on the table. Here are some
...
2. Is the frustraction cloud just a tractable approximation to 
...
partition $\hat P = [(A, B), (C, D)]$. To our graph partitioner, $\hat P$ also has a cost of 2, since positive 
weight edge crossing the partition has a cost of 2 
...
We want these these techniques to be parallelizable,
so that analysis is tractable on graphs with millions of vertices
and edges.
